    ...
    public class RegisterViewModel
    {
        ...
        public class IdentityDbInitializer
        {
            ...
            public async Task SeedAsync()
            {
                ...
                user.PasswordHash = hasher.HashPassword(user, "@Open1234"); await CreateUserAsync(user, "Admin");
                user = new IdentityUser { UserName = "r52", NormalizedUserName = "R52" };
                user.PasswordHash = hasher.HashPassword(user, "@Open1234"); await CreateUserAsync(user, "Staff");
            }
            private async Task CreateRoleAsync(IdentityRole role)
            {
                var exits = await roleManager.RoleExistsAsync(role.Name ?? ""); if (!exits)
                    await roleManager.CreateAsync(role);
            }
            private async Task CreateUserAsync(IdentityUser user, string role)
            {
                var exists = await userManager.FindByNameAsync(user.UserName ?? ""); if (exists == null)
                {
        ...
        public class IdentitySeederHostedService : IHostedService
        {
            ...
            public async Task StartAsync(CancellationToken cancellationToken)
            {
                ...
                var seeder =
               scope.ServiceProvider.GetRequiredService<IdentityDbInitializer>(); if (seeder != null)
                {
